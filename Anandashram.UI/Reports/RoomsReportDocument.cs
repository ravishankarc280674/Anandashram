...
public class RoomsReportDocument : IDocument
{
    public Company Company { get; }
    public List<RoomDTO> Rooms { get; }

    public RoomsReportDocument(Company company, List<RoomDTO> rooms)
    {
        Company = company ?? new Company();
        Rooms = rooms ?? new List<RoomDTO>();
    }

    //public DocumentMetadata GetMetadata() =>
    //    DocumentMetadata.Default.Title= "Rooms List").WithAuthor(Company?.Name ?? "Organization");
   ...
   .BorderColor(Colors.Grey.Lighten2);
    ...
    const float TableWidth = 350; // reduce total width
    const float RoomColumnWidth = 250;
    const float CapacityColumnWidth = 100;
    ...
    void DrawRoomRow(QuestPDF.Infrastructure.IContainer container, RoomDTO room)
    {
