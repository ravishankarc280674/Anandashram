namespace Anandashram.Services
{
    public class DevoteeCategoryService : IDevoteeCategoryService
    {
        ...
        public Task<PaginatedList<DevoteeCategory>> GetItems(string sortProperty, SortOrder sortOrder,
            string searchText, int pageIndex, int pageSize)
            => _repo.GetItems(sortProperty, sortOrder, searchText, pageIndex, pageSize);

        public Task<DevoteeCategory?> GetDevoteeCategory(int id)
            => _repo.GetDevoteeCategory(id);

        public IEnumerable<DevoteeCategory> GetDevoteeCategories()
            => _repo.GetDevoteeCategories();
